import { useEffect, useRef } from "react";
import { loginSchema } from "../../schemas/auth/loginSchema.js";
import { useFormik } from "formik";
import { Link } from "react-router-dom";
import useAuth from '../../hooks/useAuth.js';

const LoginModal = ({ isOpen, onClose }) => {
  const modalRef = useRef(null);
  const { login, loading } = useAuth();

  const closeModal = () => {
    onClose();
    resetForm();
  };

  const submitForm = async (values) => {
    try {
      await login(values.email, values.password);
      alert("Login exitoso");
      closeModal();
    } catch (error) {
      alert(`Error en el login: ${error}`);
    }
  };

  const { handleChange, errors, handleSubmit, values, resetForm } = useFormik({
    initialValues: {
      email: "",
      password: "",
    },
    onSubmit: submitForm,
    validationSchema: loginSchema,
  });

  useEffect(() => {
    if (isOpen) {
      modalRef.current?.showModal();
    } else {
      modalRef.current?.close();
    }
  }, [isOpen]);

  return (
    <dialog
      ref={modalRef}
      className="modal"
      onClose={closeModal}
      onClick={(e) => {
        const dialogDimensions = modalRef.current?.getBoundingClientRect();
        if (
          e.clientX < dialogDimensions.left ||
          e.clientX > dialogDimensions.right ||
          e.clientY < dialogDimensions.top ||
          e.clientY > dialogDimensions.bottom
        ) {
          onClose();
        }
      }}
    >
      <div className="modal-box" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-3xl font-bold">Iniciar Sesión</h3>
          <button
            className="btn btn-sm btn-circle btn-ghost"
            onClick={closeModal}
          >
            ✕
          </button>
        </div>
        <p className="mb-4">
          Ingresa a tu cuenta con tu dirección de correo electrónico
        </p>
        <form onSubmit={handleSubmit}>
          <label
            className={`flex items-center gap-2 input input-bordered ${
              errors.email && "input-error"
            }`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 16 16"
              fill="currentColor"
              className="w-4 h-4 opacity-70"
            >
              <path d="M2.5 3A1.5 1.5 0 0 0 1 4.5v.793c.026.009.051.02.076.032L7.674 8.51c.206.1.446.1.652 0l6.598-3.185A.755.755 0 0 1 15 5.293V4.5A1.5 1.5 0 0 0 13.5 3h-11Z" />
              <path d="M15 6.954 8.978 9.86a2.25 2.25 0 0 1-1.956 0L1 6.954V11.5A1.5 1.5 0 0 0 2.5 13h11a1.5 1.5 0 0 0 1.5-1.5V6.954Z" />
            </svg>
            <input
              type="text"
              className="grow"
              name="email"
              placeholder="tucorreo@ejemplo.com"
              onChange={handleChange}
              autoComplete="off"
              value={values.email}
            />
          </label>
          <small className="text-red-500">{errors.email}</small>
          <label
            className={`flex items-center gap-2 mt-3 input input-bordered ${
              errors.password && "input-error"
            }`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 16 16"
              fill="currentColor"
              className="w-4 h-4 opacity-70"
            >
              <path
                fillRule="evenodd"
                d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z"
                clipRule="evenodd"
              />
            </svg>
            <input
              type="password"
              className="grow"
              name="password"
              placeholder="••••••••"
              onChange={handleChange}
              autoComplete="off"
              value={values.password}
            />
          </label>
          <small className="text-red-500">
            {" "}
            <span
              className={`${errors.password ? "opacitiy-100" : "opacity-0"}`}
            >
              ❌
            </span>{" "}
            {errors.password}
          </small>
          <span className="block mt-0.5 mb-4">
            <a href="#" className="text-sm font-semibold underline">
              Olvidé mi contraseña
            </a>
          </span>
          <button
            type="submit"
            className="w-full btn btn-neutral btn-sm min-h-10"
          >
            Iniciar Sesión
          </button>
        </form>
        <div className="items-center justify-center pt-4 mx-auto mt-4 space-y-2 text-center border-t-2 border-gray-300">
          <h4 className="text-sm">
            ¿Aún no tienes cuenta? ¡Creala a continuación!
          </h4>
          <div className="flex justify-center w-full gap-2">
            <Link
              to={"/register"}
              role="button"
              className="w-1/2 btn btn-outline btn-xs"
            >
              Quiero ser voluntario
            </Link>
            <Link
              to={"/foundation/register"}
              role="button"
              className="w-1/2 btn btn-outline btn-xs"
            >
              Soy una fundacion
            </Link>
          </div>
        </div>
      </div>
    </dialog>
  );
};

export default LoginModal;
