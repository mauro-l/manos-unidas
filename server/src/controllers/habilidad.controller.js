...
export const getHabilidadById = async (req, res) => {
    try {
        ...
        if (!habilidad) return res.status(404).json({ message: 'Habilidad no encontrada' });
...
export const updateHabilidad = async (req, res) => {
    try {
        const updatedHabilidad = await Habilidad.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updatedHabilidad) return res.status(404).json({ message: 'Habilidad no encontrada' });
...
export const deleteHabilidad = async (req, res) => {
    try {
        ...
        if (!deletedHabilidad) return res.status(404).json({ message: 'Habilidad no encontrada' });
