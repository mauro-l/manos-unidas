...
export const updateVoluntario = async (req, res) => {
    try {
        const updatedVoluntario = await Voluntario.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updatedVoluntario) return res.status(404).json({ message: 'Voluntario no encontrado' });
...
export const deleteVoluntario = async (req, res) => {
    try {
        ...
        if (!deletedVoluntario) return res.status(404).json({ message: 'Voluntario no encontrado' });
